...
function openCustomerDialog(edit=null){
  const dlg = $('#dlgCustomer');
  $('#dlgCustomerTitle').textContent = edit ? '編輯客戶' : '新增客戶';
  $('#cName').value = edit?.name || '';
  $('#cPhone').value = edit?.phone || '';
  $('#cLineId').value = edit?.lineId || '';
  $('#cNote').value = edit?.note || '';
  dlg.showModal();
