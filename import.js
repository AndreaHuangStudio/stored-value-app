...
async function importCustomerCSV(customer){
  return new Promise((resolve)=>{
    ...
    input.onchange=async ()=>{
      ...
      const idxDate=header.indexOf('日期'), idxType=header.indexOf('類型'), idxAmt=header.indexOf('金額'), idxNote=header.indexOf('備註'), idxLM=header.indexOf('最後修改');
      if(idxDate<0 || idxType<0 || idxAmt<0){ alert('CSV 欄位需包含：日期、類型、金額（備註/最後修改可選）'); resolve({imported:0, skipped:rows.length-1, duplicated:0}); return; }
      ...
      for(let i=1;i<rows.length;i++){ const r=rows[i];
        ...
        await saveTxn({customerId:customer.id, type:t, amount:a, note:n, createdAt:d, lastModified: lm || Date.now()});
        seen.add(fp); ok++;
      }
