...
function openDB(){ if(dbPromise) return dbPromise; dbPromise=new Promise((resolve,reject)=>{
  ...
  req.onupgradeneeded=()=>{ const db=req.result; let s;
    ...
    if(!db.objectStoreNames.contains(STORE_INTERVIEWS)){ const i=db.createObjectStore(STORE_INTERVIEWS,{keyPath:'id'});
      i.createIndex('customerId','customerId',{unique:false}); i.createIndex('date','date',{unique:false}); }
  };
  req.onsuccess=()=>resolve(req.result); req.onerror=()=>reject(req.error);
}); return dbPromise; }
async function dbPut(store,value){ const db=await openDB(); return new Promise((res,rej)=>{ const tx=db.transaction(store,'readwrite'); tx.objectStore(store).put(value); tx.oncomplete=()=>res(value); tx.onerror=()=>rej(tx.error);});}
async function dbGetAll(store,indexName=null,query=null){ const db=await openDB(); return new Promise((res,rej)=>{ const tx=db.transaction(store,'readonly'); const os=tx.objectStore(store); const src=indexName?os.index(indexName):os; const req=src.getAll(query); req.onsuccess=()=>res(req.result); req.onerror=()=>rej(req.error);});}
