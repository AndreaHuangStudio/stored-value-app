...
async function importAllTxnsCSV(){
  ...
  const cacheSets = new Map(); // customerId -> Set(fingerprint)
  ...
  for(let i=1;i<rows.length;i++){ const r=rows[i];
    ...
    await saveTxn({customerId:c.id, type:t, amount:a, note:n, createdAt:d});
    set.add(fp); ok++;
  }
...
async function importAllInterviewsCSV(){
  ...
  for(let i=1;i<rows.length;i++){ const r=rows[i];
    ...
    if(set.has(fp)){ dup++; continue; }
    await saveInterview({customerId:c.id, date:d, topic, content, nextAction});
    set.add(fp); ok++;
  }
...
async function importAllDataJSON(){
  ...
  // 建立每位客戶的現有集合（延後載入）
  const cacheTx = new Map(); // id -> Set
  const cacheIv = new Map(); // id -> Set
  ...
  for(const block of arr){
    ...
    for(const t of (block.txns||[])){
      ...
      await saveTxn({customerId:c.id, type:t.type, amount:Number(t.amount||0), note:t.note||'', createdAt: Number(t.createdAt)||Date.parse(t.createdAt)||Date.now()});
      setT.add(fp); cntT++;
    }
    for(const it of (block.interviews||[])){
      ...
      await saveInterview({customerId:c.id, date: when, topic:it.topic||'', content:it.content||'', nextAction:it.nextAction||''});
      setI.add(fp); cntI++;
    }
    cntC++;
  }
